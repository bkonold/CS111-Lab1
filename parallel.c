...
void execute_parallel(command_stream_t commandStream) {
    ...
    *numFreeProcesses = MAX_PROCS;
    ...
    mutp = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    ...
    while (currNode) {
        ...
        while (*numFreeProcesses < 1) {
            // wait for a child to finish beore forking again
        ...
        if (p == 0) {
           ...
           // printf("\nparent aquired lock\n");
            (*numFreeProcesses)--;
            //printf("(parent)numFreeProcesses is %d\n", *numFreeProcesses);
           // printf("\nparent released lock\n");
            pthread_mutex_unlock (mutp);
            currNode = currNode->next;
        }
        else if (p == -1) {
            // forked too many times, need to wait!
            wait(NULL);
        }
        i++;
    }
...
execute_node(graphnode_t node) {
    ...
    //printf("\nchild aquired lock\n");
    (*numFreeProcesses)++;
    //printf("(child)numFreeProcesses is %d\n", *numFreeProcesses);
